...
//		} else {
			for (int i = 1; i <= weeks; i++) {
				GregorianCalendar weekEndDate = addDays(
						(GregorianCalendar) weekStartDate.clone(), 7);
...
//		}
	  ...
	  public static void main(String[] args) { 
		  ...
		  sb.append("Can someone please tell me how to get a Text value out of a Google App Engine datastore using Java? I have some entities in the datastore with a Text property named longDescription. When I try this:");
		  sb.append("Can someone please tell me how to get a Text value out of a Google App Engine datastore using Java? I have some entities in the datastore with a Text property named longDescription. When I try this:");
		  sb.append("Can someone please tell me how to get a Text value out of a Google App Engine datastore using Java? I have some entities in the datastore with a Text property named longDescription. When I try this:");
		  sb.append("Can someone please tell me how to get a Text value out of a Google App Engine datastore using Java? I have some entities in the datastore with a Text property named longDescription. When I try this:");
		  sb.append("Can someone please tell me how to get a Text value out of a Google App Engine datastore using Java? I have some entities in the datastore with a Text property named longDescription. When I try this:");
	...
	public Date[] getLastYearDate() {
		...
		GregorianCalendar endDate = addDays(
				(GregorianCalendar) prevYear.clone(), 6);
		setEndOfDay(endDate);
	...
	public Date[] getLastHalfDate() {
		...
		if (week > 26) {
			...
			GregorianCalendar prevYearEndWeek = addDays(
					convertDateToCalendar(getCurrentYearStartDate()), -7);
	...
	public Date[] getLastTradingPeriodDate() {
		...
		if (tradingPeriodNumber > 0) {
			...
			GregorianCalendar startDate = convertDateToCalendar((Date) getCurrentYearStartDate()
					.clone());
			addDays(startDate, -7);
			GregorianCalendar endDate = convertDateToCalendar((Date) getCurrentYearStartDate()
					.clone());
			addDays(endDate, -1);
			setEndOfDay(endDate);
	...
	public Date[] getCurrentWeekDates() {
		...
		GregorianCalendar weekEndDate = addDays(
				(GregorianCalendar) weekStartDate.clone(), 6);
	...
	public Date[] getLastWeekDates() {
		GregorianCalendar weekStartDate = getWeekStartDate(convertDateToCalendar(new Date()));
		GregorianCalendar prevWeekStartDate = addDays(
				(GregorianCalendar) weekStartDate.clone(), -7);
		GregorianCalendar prevWeekEndDate = addDays(
				(GregorianCalendar) prevWeekStartDate.clone(), 6);
		setEndOfDay(prevWeekEndDate);
		return new Date[] { prevWeekStartDate.getTime(),
				prevWeekEndDate.getTime() };
	}
	...
	public boolean isEndOfTradingPeriod(Date date) {
		...
		cacheValue = new Boolean(week == 0 || week == 4 || week == 8 || week == 13 || week == 17
				|| week == 21 || week == 26 || week == 30 || week == 34
				|| week == 39 || week == 43 
				|| weeksCount == 52 && (week == 47 || week == 52)
				|| weeksCount == 53 && (week == 48 || week == 53));
		datePeriodCache.put(date, cacheValue);		
		return (week == 0 || week == 4 || week == 8 || week == 13 || week == 17
				|| week == 21 || week == 26 || week == 30 || week == 34
				|| week == 39 || week == 43 
				|| weeksCount == 52 && (week == 47 || week == 52)
				|| weeksCount == 53 && (week == 48 || week == 53));
	...
	public boolean isEndOfHalf(Date date) {
		...
		cacheValue = new Boolean((week == 26|| (weeksCount==52 && week==52) || (weeksCount==53 && week==53)));
		dateHalfCache.put(date, cacheValue);		
	...
	public boolean isEndOfYear(Date date) {
		...
		cacheValue = new Boolean((weeksCount == 52 && week == 52 || weeksCount == 53
				&& week == 53));
		dateYearCache.put(date, cacheValue);
		return (weeksCount == 52 && week == 52 || weeksCount == 53
				&& week == 53);
	}
	...
	public ArrayList<Date> getTradingYearWeeks() {
		...
		GregorianCalendar calendar = convertDateToCalendar((Date) getCurrentYearStartDate()
				.clone());
	...
	public static GregorianCalendar addMonth(GregorianCalendar calendar,
			int month) {
	...
	public void getDateRangesForPatch(List<Date> afterDates, List<Date> beforeDates){
		try {
			...
			for(int i = 0 ; i <160;i++){
				...
				if(isEndOfTradingPeriod(dayAfter.getTime())
						|| isEndOfHalf(dayAfter.getTime())
	...
	public static String getDateToString(Date date) {
		...
		if (date != null) {
			...
			GregorianCalendar yesterday = addDays(
					convertDateToCalendar(new Date()), -1);
	...
	public static Date addDaysToTime(Date date, int minute,int secund) {
		return new Date(date.getYear(), date.getMonth(), date.getDate(),
				date.getHours(), date.getMinutes()+minute, date.getSeconds()+secund);
	}
