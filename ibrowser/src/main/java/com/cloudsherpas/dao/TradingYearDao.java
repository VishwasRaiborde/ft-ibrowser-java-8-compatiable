...
public class TradingYearDao extends BaseDao<TradingYear> {
	...
	public List<TradingYear> getTradingYearsByParam(String sortKey,
			String order, String step) {
		PersistenceManager pm = getPersistenceManager();
		Query q = pm.newQuery(TradingYear.class);
	...
	public TradingYear getTradingYearByDate(Date date) {
		...
		if (list == null || list.isEmpty()) {
			logger.log(logger.getLevel().SEVERE, "No trading period found for the date:" + DateUtils.dateFormat1.format(date));
			return null;
		}
		...
		try {
			...
			for (TradingYear tradingYear : list) {
				if (tradingYear.getStartDate().compareTo(date) != 1 && tradingYear.getEndDate().compareTo(date) == 1) {
					return tradingYear;
				}
			}
			
			//createTradingYearByPrevNexYears(date, true); // no need to create the trading years automatically,
														   //Trading years must be manually added in the admin section
			logger.log(Level.SEVERE, "No Trading Year found for the date:" + DateUtils.dateFormat1.format(date));
	...
	protected void createTradingYearByPrevNexYears(Date date, boolean greaterThan) {
		...
		try {
			...
			} while(!(tradingYear.getStartDate().compareTo(date) != 1 && tradingYear.getEndDate().compareTo(date) == 1));
