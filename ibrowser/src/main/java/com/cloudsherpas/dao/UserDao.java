...
public class UserDao extends BaseDao<AppUser> {
  ...
  /*
   * public void saveUserFavouriteReport(Report favouriteReport, Boolean isFavourite) { AppUser user
   * = getCurrentUser(); if (isFavourite) {
   * user.getFavouriteReportsKey().add(favouriteReport.getKey()); } else {
   * user.getFavouriteReportsKey().remove(favouriteReport.getKey()); } persist(user); } public
   * List<Key> getUserFavouriteReportKey() { return getCurrentUser().getFavouriteReportsKey(); }
   */
