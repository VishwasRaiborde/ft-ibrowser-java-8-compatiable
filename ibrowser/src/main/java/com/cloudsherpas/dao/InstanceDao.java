...
public class InstanceDao extends BaseDao<Instance> {
	...
	public List<Instance> getReportDates(String reportKey, String groupCode, ReportPeriodEnum reportPeriodEnum) {
		return getReportDates(reportKey, groupCode, reportPeriodEnum, new Date());
	}
	
	public List<Instance> getReportDates(String reportKey, String groupCode, ReportPeriodEnum reportPeriodEnum, Date date) {
		PersistenceManager pm = getPersistenceManager();

		Query instanceQuery = pm.newQuery(Instance.class);
		instanceQuery.setFilter("period == :periodParam && reportCode == :reportCodeParam && groupCode == :groupCode && date <= :dateParam");
		instanceQuery.setOrdering("date DESC");
		try {
			List<Instance> instanceList = (List<Instance>) instanceQuery.executeWithArray(reportPeriodEnum, reportKey, groupCode, date);
			return instanceList;
		} finally {
//			instanceQuery.closeAll();
	...
	public List<Instance> getReportDates(String reportKey, ReportPeriodEnum reportPeriodEnum, Date date) {
		PersistenceManager pm = getPersistenceManager();

		Query instanceQuery = pm.newQuery(Instance.class);
		instanceQuery.setFilter("period == :periodParam && reportCode == :reportCodeParam && date <= :dateParam");
		instanceQuery.setOrdering("date DESC");
		try {
			List<Instance> instanceList = (List<Instance>) instanceQuery.executeWithArray(reportPeriodEnum, reportKey, date);
			return instanceList;
		} finally {
//			instanceQuery.closeAll();
			pm.close();
	...
	public List<Instance> getInstancesByRangeForDelete(String reportCode, Date dateBefore, int startIndex, int endIndex) {
		PersistenceManager pm = getPersistenceManager();
		Query q = pm.newQuery(Instance.class);
		q.setFilter("reportCode == :codeParam && (period == '"+ReportPeriodEnum.WEEKLY+"' || period == '"+ReportPeriodEnum.DAILY+"') && date <= :dateBeforeParam");
	...
	public boolean hasEndOfPeriodInstances(String fileName) {
		PersistenceManager pm = getPersistenceManager();
		Query q = pm.newQuery(Instance.class);
		q.setFilter("fileName == :fileNameParam && period == '"+ReportPeriodEnum.END_OF_TRADING_PERIOD+"'");
		q.setRange(0,1);
		try {
	...
	public List<Instance> getLastInstances(String code, String groupCode, ReportPeriodEnum reportPeriodEnum) {
		PersistenceManager pm = getPersistenceManager();

		Query instanceQuery = pm.newQuery(Instance.class);
		instanceQuery.setFilter("period == :periodParam && reportCode == :reportCodeParam && groupCode == :groupCodeParam && date <= :dateParam");
		instanceQuery.setOrdering("date DESC");
		...
		try {
			List<Instance> instanceList = (List<Instance>) instanceQuery.executeWithArray(reportPeriodEnum, code, groupCode, new Date());
			return instanceList;
		} finally {
//			instanceQuery.closeAll();
			pm.close();
	...
	public List<Instance> getLastTradingPeriodInstances(String code, String groupCode, ReportPeriodEnum reportPeriodEnum, Date[] range) {
		PersistenceManager pm = getPersistenceManager();

		Query instanceQuery = pm.newQuery(Instance.class);
		instanceQuery.setFilter("period == :periodParam && reportCode == :reportCodeParam && groupCode == :groupCodeParam && date >= :dateParam1 && date <= :dateParam2");
		instanceQuery.setOrdering("date DESC");
		...
		try {
			List<Instance> instanceList = (List<Instance>) instanceQuery.executeWithArray(reportPeriodEnum, code, groupCode, range[0], range[1]);
			return instanceList;
		} finally {
//			instanceQuery.closeAll();
			pm.close();
	...
	public List<Instance> getInstancesByAfterTheDate( ReportPeriodEnum reportPeriodEnum, /*Date afterTheDate, Date beforeTheDate,*/ long start, long end) {
		PersistenceManager pm = getPersistenceManager();
		Query instanceQuery = pm.newQuery(Instance.class);
		instanceQuery.setFilter("period ==:periodParam " /*&& date >= :dateParam && date < :dateParam2 */);
		instanceQuery.setOrdering("date DESC");
		instanceQuery.setRange(start, end);
		try {
			List<Instance> instanceList = (List<Instance>) instanceQuery.executeWithArray(reportPeriodEnum);
			return instanceList;
		} finally {
//			instanceQuery.closeAll();
			pm.close();
