...
public class UserFavouriteReportDao extends BaseDao<UserFavouriteReport>{
	...
	public void saveRemoveUserFavouriteReport(String userKey,String reportKey,String groupCode,String title,Boolean isFavourite) {
		...
		if (isFavourite) {
		   ...
		   favouriteReport = getUserFavouriteReport(userKey,reportKey,groupCode);
		    if(favouriteReport!=null) {
			   ...
			   logger.log(Level.INFO, "User Favourite Report Successful Deleted! Group Code: " + groupCode);
		    }
		 }
			
	}
	public UserFavouriteReport getUserFavouriteReport(String userKey,String reportKey,String groupCode) {
		PersistenceManager pm = getPersistenceManager();
		Query q = pm.newQuery(UserFavouriteReport.class);
		q.setFilter("userKey == :userKeyParam && reportKey == :reportKeyParam && groupCode == :groupCodeParam");
		 try {
			List<UserFavouriteReport> list = (List<UserFavouriteReport>) q.execute(userKey,reportKey,groupCode);
			 if (list.isEmpty()) {
					return null;
				}
				return list.get(0);
