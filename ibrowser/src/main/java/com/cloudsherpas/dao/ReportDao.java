...
public class ReportDao extends BaseDao<Report> {
  ...
  public ReportDao(UserDao userDao, GroupDao groupDao, HeadingDao headingDao,
      InstanceDao instanceDao, GoogleCloudStorageApi cloudStorageApi, MemcacheService cache) {
    super(Report.class);
  ...
  public List<Report> getDivisionalReport(String headingKey) {
    ...
    q.setFilter(
        "heading == :headingParam && allowedGroupsKey == :allowGroupParam && reportType == :reportTypeParam");
    ...
    try {
      ...
      for (int i = 0; i < a; i++) {
        ...
        List<Report> subList = (List<Report>) q.execute(heading, subGroups,
            ReportTypeEnum.DIVISIONAL);
  ...
  public List<Report> getReports(String headingKey, ReportTypeEnum reportType,
    ...
    Query q = pm.newQuery(Report.class);
    q.setFilter(
        "heading == :headingParam && reportType == :typeParam && allowedGroupsKey == :allowedGroupsParam");
  ...
  public List<Report> getSearchReports(ReportTypeEnum reportType, String typeGroupCode,
      String codeSearchVal, String titleSearchVal) {
    List<Report> reportList = new ArrayList<Report>();
    List<Key> userGroupsKey = groupDao.getUserGroupsKey(userDao.getCurrentUser());
    ...
    try {
      ...
      logger.log(Level.INFO, "Branch/Buying report list start :  Time: " + new Date());
      for (Report report : list) {
        if (report.getCode().contains(codeSearchVal)
            || report.getTitle().contains(titleSearchVal)) {
          Instance instance = instanceDao.getLastInstanceByCode(report.getCode(), typeGroupCode);
          if (instance != null) {
            report.setKeyAsString();
            report.setLastReportDateAsString(DateUtils.getDateFormat(instance.getDate()));
  ...
  /*
   * public List<Report> getUserFavouriteReports() { PersistenceManager pm =
   * getPersistenceManager(); List<Key> userFavouriteReports =
   * userDao.getCurrentUser().getFavouriteReportsKey();
   * 
   * if (userFavouriteReports == null || userFavouriteReports.isEmpty()) { return new
   * ArrayList<Report>(); }
   * 
   * Query q = pm.newQuery(Report.class); q.setFilter("key == :keyParam");
   * q.setOrdering("title ASC,lastUpdatedDate DESC"); try { List<Report> list = (List<Report>)
   * q.execute(userFavouriteReports); if (list.isEmpty()) { return new ArrayList<Report>(); } for
   * (Report report : list) { Instance instance =
   * instanceDao.getLastInstanceByCode(report.getCode(), null); report.setKeyAsString();
   * report.setGroupCode(instance != null ? instance.getGroupCode() : null); } return list; }
   * finally { q.closeAll(); pm.close(); } }
   */
  ...
  public List<SelectItem> getReportTypeGroups(Report report) {
    ...
    try {
      ...
      instanceGroupCodes.addAll(instanceDao.getInstanceGroupCodes(report.getCode()));
      if (instanceGroupCodes != null && !instanceGroupCodes.isEmpty()) {
        List<Type> types = (List<Type>) q.execute(new ArrayList<String>(instanceGroupCodes),
  ...
  public List<Report> getDivisionalReports(String code, String title) {
    ...
    try {
      ...
      logger.log(Level.INFO, "Divisional report list start :  Time: " + new Date());
      for (Report report : list) {
        if (report.getCode().toUpperCase().contains(code)
            || report.getTitle().toUpperCase().contains(title)) {
          Instance instance = instanceDao.getLastInstanceByCode(report.getCode(), "000");
          if (instance != null) {
            report.setKeyAsString();
            report.setTypeAsString(report.getReportType().getName());
  ...
  public void delete(Object key) {
    ...
    try {
      ...
      List<Instance> reportWeeklyInstances = (List<Instance>) q.execute(reportCode);
      ArrayList<String> fileNames = new ArrayList<String>();
      if (reportWeeklyInstances != null && !reportWeeklyInstances.isEmpty()) {
