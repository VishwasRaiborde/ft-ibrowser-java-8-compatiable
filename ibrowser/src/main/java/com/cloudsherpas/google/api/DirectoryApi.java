...
public class DirectoryApi implements GlobalConstants {
  ...
  public DirectoryApi() {
    try {
      ...
      GoogleCredential credential = new GoogleCredential.Builder()
          ...
          .setServiceAccountId(SERVICE_ACCOUNT_ID).setServiceAccountUser(ACCOUNT_USER)
          .setServiceAccountScopes(SCOPES).setServiceAccountPrivateKeyFromP12File(new File(this
              .getClass().getClassLoader().getResource(SERVICE_ACCOUNT_PKCS12_FILE_PATH).getPath()))
          .build();

      HttpTransport httpTransport = new NetHttpTransport();
      JsonFactory jsonFactory = JacksonFactory.getDefaultInstance();
      admin = new Directory.Builder(httpTransport, jsonFactory, credential)
          .setApplicationName("iBrowser").setHttpRequestInitializer(credential).build();
    } catch (Exception e) {
      e.printStackTrace();
    }
  ...
  public String checkGroupMember(String email) {
    ...
    try {
      ...
      for (Group group : groups.getGroups()) {
        ...
        com.google.api.services.admin.directory.Directory.Members.Get item = admin.members()
            .get(group.getEmail(), email);
  ...
  public void getMembers(String reportGroup, String email) {
    try {
      com.google.api.services.admin.directory.Directory.Members.List list = admin.members()
          .list(reportGroup);
    } catch (Exception e) {
      e.printStackTrace();
    }
