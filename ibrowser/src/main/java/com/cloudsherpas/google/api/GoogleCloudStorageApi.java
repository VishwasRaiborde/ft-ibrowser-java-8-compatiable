...
public class GoogleCloudStorageApi implements GlobalConstants {
  ...
  public GoogleCloudStorageApi(InstanceDao instanceDao, TradingYearDao tradingYearDao,
  ...
  public void init() {
    try {
      ...
      List<String> SCOPE = Arrays.asList("https://www.googleapis.com/auth/admin.directory.group",
      ...
      GoogleCredential CREDENTIAL = new GoogleCredential.Builder().setTransport(transport)
          .setJsonFactory(jsonFactory).setServiceAccountId(SERVICE_ACCOUNT_ID)
...
//							DateUtils dateUtils = new DateUtils(tradingYear.getStartDate());
              ...
              /*
               * if (dateUtils.isEndOfWeek(instance.getDate()) &&
               * ReportPeriodEnum.DAILY.equals(reportPeriod)) { reportPeriod =
               * ReportPeriodEnum.WEEKLY; }
               */

              if (dateUtils.isEndOfYear(new Date(instance.getDate().getTime()))
                  && ReportPeriodEnum.WEEKLY.equals(reportPeriod)) {
...
//								instances.add(createInstance(strings, ReportPeriodEnum.END_OF_HALF, object));
              ...
              if (dateUtils.isEndOfHalf(new Date(instance.getDate().getTime()))
                  && ReportPeriodEnum.WEEKLY.equals(reportPeriod)) {
//								instances.add(createInstance(strings, ReportPeriodEnum.END_OF_TRADING_PERIOD, object));
                instances.add(createInstance(strings, ReportPeriodEnum.END_OF_HALF, object));
              }
              if (dateUtils.isEndOfTradingPeriod(new Date(instance.getDate().getTime()))
                  && ReportPeriodEnum.WEEKLY.equals(reportPeriod)) {
                instances
                    .add(createInstance(strings, ReportPeriodEnum.END_OF_TRADING_PERIOD, object));
              }
              // move file to report bucket,remove from incoming
              moveFileToReportBucket(object);

              logger.log(Level.INFO,
                  "GoogleCloudStorageApi run method. moveFileToReportBucket(" + name + ")");
            } else {
              try {
                errorFiles.add(object.getName());
                moveFileToErrorBucket(object);
                logger.log(Level.INFO, "GoogleCloudStorageApi run method. moveFileToErrorBucket("
                    + name + "). No report for this code.");
              } catch (Exception e) {
                continue;
              }
            }
            ...
            try {
              moveFileToErrorBucket(object);
              logger.log(Level.SEVERE, "GoogleCloudStorageApi run method. moveFileToErrorBucket("
                  + object.getName() + ")" + e.getMessage());
            } catch (Exception e1) {
              continue;
            }
          }
    ...
    if (!noInsranceFiles.isEmpty()) {
      ...
      notificationDao.saveNotification(FROM_EMAIL, TO_EMAIL, "Report fetching notification",
          body.toString());
  ...
  private Instance createInstance(String[] strings, ReportPeriodEnum period, StorageObject object)
  ...
  private void moveFileToReportBucket(StorageObject object) throws IOException {
    ...
    for (int i = 0; i < 5; i++) {
      try {
        storage.objects().copy(INCOMING_BUCKET, object.getName(), REPORTS_BUCKET,
            (strings[1] + "/" + strings[0] + "/" + object.getName()), object).execute();
        logger.log(Level.INFO, "File \"" + object.getName() + "\" successfully copied to "
        ...
        logger.log(Level.INFO, "Error encountered in moving files to Report Bucket. "
            + object.getName() + ". Retrying (" + (i + 1) + ")");
        if (i > 4) {
          break;
        }
      }
  ...
  private void moveFileToErrorBucket(StorageObject object) throws IOException {
    ...
    storage.objects()
        .copy(INCOMING_BUCKET, object.getName(), ERRORS_BUCKET, object.getName(), object).execute();
    logger.log(Level.INFO,
        "File \"" + object.getName() + "\" successfully copied to " + ERRORS_BUCKET + " bucket.");

    storage.objects().delete(INCOMING_BUCKET, object.getName()).execute();
    logger.log(Level.INFO, "File \"" + object.getName() + "\" successfully copied from "
        + INCOMING_BUCKET + " bucket.");
  }
    ...
    if (fileNames != null && !fileNames.isEmpty()) {
      try {
        ...
        for (String fileName : fileNames) {
          ...
          storage.objects().copy(REPORTS_BUCKET, storageObject.getName(), DELETED_BUCKET,
              (strings[0] + "- deleted " + deletedDate + "/" + fileName), null).execute();
          logger.log(Level.INFO,
              "File \"" + fileName + "\" successfully copied to " + DELETED_BUCKET + " bucket.");

          storage.objects().delete(REPORTS_BUCKET, filePath).execute();
          logger.log(Level.INFO,
              "File \"" + fileName + "\" successfully deleted from " + REPORTS_BUCKET + " bucket.");
