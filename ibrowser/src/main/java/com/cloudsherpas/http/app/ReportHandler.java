...
public class ReportHandler {
  ...
  @Inject
  public ReportHandler(ReportDao reportDao, UserDao userDao, HeadingDao headingDao,
      UserFavouriteReportDao userFavouriteReportDao, ApiHttpUtils utils) {
  ...
  public String get(String headingKey, HttpServletRequest req, HttpServletResponse resp)
      throws IOException {
    ...
    List<UserFavouriteReport> userFavouriteReports = userFavouriteReportDao
        .getUserFavouriteReports(currentUser.getKeyAsString());
    ...
    if (userBranches != null && !userBranches.isEmpty() && userBranches.size() == 1) {
      ...
      req.setAttribute("branchReport",
          reportDao.getReports(headingKey, ReportTypeEnum.BRANCH, userBranches.get(0).getKey()));
    }
    if (currentUser.getBuyingOfficeCode() != null
        && !"".equals(currentUser.getBuyingOfficeCode())) {
      req.setAttribute("userSavedBO",
          new SelectItem(currentUser.getBuyingOfficeCode(), currentUser.getBuyingOfficeCode()));
      req.setAttribute("buyingReport", reportDao.getReports(headingKey, ReportTypeEnum.BUYING,
          currentUser.getBuyingOfficeCode()));
    }
    return "OK";
  }

  public void post(String headingKey, HttpServletRequest req, HttpServletResponse resp)
      throws IOException {
//		logger.log(Level.INFO,"On ReportHandler Post");
    CreateReportRequest reportReq = (CreateReportRequest) utils.readPost(req,
        CreateReportRequest.class);
    List<Report> reports = reportDao.getReports(headingKey,
        ReportTypeEnum.buildByCode(reportReq.type), reportReq.key);

    int retries = 0;
    while (reports.size() < 1 && retries < 10) {
      reports = reportDao.getReports(headingKey, ReportTypeEnum.buildByCode(reportReq.type),
          reportReq.key);
      retries++;
    }
