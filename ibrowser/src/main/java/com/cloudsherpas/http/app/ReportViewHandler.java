...
public class ReportViewHandler {
  ...
  @Inject
  public ReportViewHandler(UserDao userDao, ReportDao reportDao, InstanceDao instanceDao,
      AuditDao auditDao, MemcacheService cache, GroupDao groupDao) {
  ...
  public void get(String key, HttpServletRequest req, HttpServletResponse resp) throws Exception {
    ...
    if (report != null) {
      ...
      logger.log(Level.INFO,
          "Report instance :" + reportInstance == null ? "NULL" : reportInstance.getFileName());
    ...
    if (report != null && isAllowReportToUser(report, currentUser)) {
      ...
      String fileName = "/" + reportInstance.getGroupCode() + "/" + reportInstance.getReportCode()
          + "/" + reportInstance.getFileName();
