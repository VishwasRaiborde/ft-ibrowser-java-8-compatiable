...
public class ReportDatesHandler {
  ...
  @Inject
  public ReportDatesHandler(InstanceDao instanceDao, ReportDao reportDao, UserDao userDao,
      TradingYearDao tradingYearDao, AuditDao auditDao, GroupDao groupDao, ApiHttpUtils utils,
  ...
  public void get(String id, HttpServletRequest req, HttpServletResponse resp) throws Exception {
    ...
    if (report != null && isAllowReportToUser(report, currentUser)) {
      ...
      gcsApi.init();
      String fileFullPath = "/" + instance.getGroupCode() + "/" + instance.getReportCode() + "/"
  ...
  public void post(HttpServletRequest req, HttpServletResponse resp) throws IOException {
    ...
    TradingYear currentYear = tradingYearDao
        .getTradingYearByDate(DateUtils.convertDateToCalendar(new Date()).getTime());
//		DateUtils dateUtils = new DateUtils(currentYear.getStartDate());
    ...
    if (instanceRequest != null && "simple".equals(instanceRequest.url)) {
      ...
      List<Instance> currentWeekPeriodDate = instanceDao.getLastInstances(instanceRequest.code,
          instanceRequest.groupCode,
          FrequencyEnum.DAILY.equals(report.getFrequency()) ? ReportPeriodEnum.DAILY
      ...
      if (currentWeekPeriodDate != null && !currentWeekPeriodDate.isEmpty()) {
        ...
        if (FrequencyEnum.WEEKLY.equals(result.getFrequency())
            && item.getDate().compareTo(dateUtils.getLast7DaysPeriod()[0]) != 1
            && item.getDate().compareTo(dateUtils.getLast7DaysPeriod()[1]) != -1) {
          result.setLastWeek(item);
          result.getHistories().remove(item);
        } else if (FrequencyEnum.DAILY.equals(result.getFrequency()) && item.getDate()
            .compareTo(DateUtils.convertDateToCalendar(new Date()).getTime()) == 0) {
          result.setToday(item);
          result.getHistories().remove(item);
        }
      }

      List<Instance> lastTradingPeriodDates = instanceDao.getLastTradingPeriodInstances(
          instanceRequest.code, instanceRequest.groupCode, ReportPeriodEnum.END_OF_TRADING_PERIOD,
          dateUtils.getLastTradingPeriodDate());

      if (lastTradingPeriodDates != null && !lastTradingPeriodDates.isEmpty()) {
        result.setLastTradingPeriod(lastTradingPeriodDates.get(0));
      }

      List<Instance> lastHalfDates = instanceDao.getLastTradingPeriodInstances(instanceRequest.code,
          instanceRequest.groupCode, ReportPeriodEnum.END_OF_HALF, dateUtils.getLastHalfDate());

      if (lastHalfDates != null && !lastHalfDates.isEmpty()) {
        result.setLastHalf(lastHalfDates.get(0));
      }

      List<Instance> lastYearDates = instanceDao.getLastTradingPeriodInstances(instanceRequest.code,
          instanceRequest.groupCode, ReportPeriodEnum.END_OF_YEAR, dateUtils.getLastYearDate());
      ...
      List<Instance> reportHistories = instanceDao.getReportDates(instanceRequest.code,
          instanceRequest.groupCode,
          FrequencyEnum.DAILY.equals(report.getFrequency()) ? ReportPeriodEnum.DAILY
              : ReportPeriodEnum.WEEKLY);
      ...
      List<Instance> reportTradingPeriodHistories = instanceDao.getReportDates(instanceRequest.code,
          instanceRequest.groupCode, ReportPeriodEnum.END_OF_TRADING_PERIOD);
      result.setTradingPeriods(reportTradingPeriodHistories);

      List<Instance> reportEndOfHalfHistories = instanceDao.getReportDates(instanceRequest.code,
          instanceRequest.groupCode, ReportPeriodEnum.END_OF_HALF);
      result.setHalfs(reportEndOfHalfHistories);

      List<Instance> reportEndOfYearHistories = instanceDao.getReportDates(instanceRequest.code,
          instanceRequest.groupCode, ReportPeriodEnum.END_OF_YEAR);
