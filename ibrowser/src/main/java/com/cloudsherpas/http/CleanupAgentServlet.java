...
public class CleanupAgentServlet extends HttpServlet implements GlobalConstants {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		while (reports != null) {
			...
			for (Report report : reports) {
				...
				instances = instanceDao.getInstancesByRangeForDelete(report.getCode(), dateBefore, start, end);
							...
							if (instanceDao.hasEndOfPeriodInstances(instance.getFileName())) {
								...
								if (removeInstanceFromBucket(instance)) {
									...
									logger.log(Level.INFO,
											"Daily Instance successfull daleted from Datastore");
								} else{
									errorFiles.add(instance.getFileName());
								}
		...
		if (!errorFiles.isEmpty()) {
			...
			notificationDao.saveNotification(FROM_EMAIL, TO_EMAIL, "Clean agent run failed", body.toString());			
		}
	}
	...
	private boolean removeInstanceFromBucket(Instance instance) {
		...
		String filePath = instance.getGroupCode() + "/"+ instance.getReportCode() + "/" + instance.getFileName();
		try {
			Storage.Objects.Get getObject = cloudStorageApi.storage.objects().get(REPORTS_BUCKET, filePath);
			StorageObject storageObject = getObject.execute();
			cloudStorageApi.storage.objects().copy(REPORTS_BUCKET,storageObject.getName(),DELETED_BUCKET,
							(instance.getReportCode() + "- deleted "+ deletedDate + "/" + instance.getFileName()), null).execute();
			logger.log(Level.INFO, "File \"" + instance.getFileName()+ "\" successfully cop	ied to " + DELETED_BUCKET+ " bucket.");

			cloudStorageApi.storage.objects().delete(REPORTS_BUCKET, filePath).execute();
			logger.log(Level.INFO, "File \"" + instance.getFileName()+ "\" successfully deleted from " + REPORTS_BUCKET+ " bucket.");
		} catch (IOException e) {
			e.getMessage();
			return false;
		}
