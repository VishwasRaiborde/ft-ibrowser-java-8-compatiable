...
public class UserAgentInfo {
  ...
  public static final String mobile = "mobile"; // Some mobile browsers put "mobile" in their user
                                                // agent strings.
  public static final String mobi = "mobi"; // Some mobile browsers put "mobi" in their user agent
                                            // strings.
  ...
  /**
   * Initialize the userAgent and httpAccept variables by getting the headers from the
   * HttpServletRequest
   *
  ...
  /**
   * Detects window OS PC
   */
  public boolean detectWindowsPC() {
    return userAgent.indexOf(deviceWindows) != -1;
  }

  /**
   * Detects Mac OS PC
   */
  public boolean detectMacPC() {
    return userAgent.indexOf(deviceWindows) != -1;
  }
  ...
  /**
   * Detects if the current browser is a BlackBerry Touch device, such as the Storm
   */
  ...
  public boolean detectSmartphone() {
    return (detectIphoneOrIpod() || detectS60OssBrowser() || detectSymbianOS()
        || detectWindowsMobile() || detectBlackBerry() || detectPalmOS() || detectPalmWebOS()
        || detectAndroid());
  }
  ...
  /**
   * The quick way to detect for a mobile device. Will probably detect most recent/current mid-tier
   * Feature Phones as well as smartphone-class devices.
   */
   ...
   * display iPhone-optimized web content. Includes iPhone, iPod Touch, Android, Palm WebOS, etc.
   */
  ...
  /**
   * The quick way to detect for a tier of devices. This method detects for all smartphones, but
   ...
   * phones, but excludes the iPhone and Smartphone Tier devices.
   */
  ...
  public String getDeviceType() {
    ...
    if (detectAndroidWebKit()) {
      return deviceAndroid + " " + engineWebKit;
    }
    if (detectWebkit()) {
      return engineWebKit;
    }
    ...
    if (detectBlackBerryTouch()) {
      return deviceBBStorm;
    } else if (detectPalmOS()) {
      return devicePalm;
    } else if (detectPalmWebOS()) {
      return deviceWebOS;
    }
    if (detectBrewDevice()) {
      return deviceBrew;
    ...
    if (detectOperaMobile()) {
      return engineOpera + " " + mobi;
    }
    if (detectWapWml()) {
      return vndwap;
    }
    if (detectSonyPlaystation()) {
      return devicePlaystation;
    }
    if (detectNintendo()) {
      return deviceNintendo;
    }
    if (detectXbox()) {
      return deviceXbox;
    }
