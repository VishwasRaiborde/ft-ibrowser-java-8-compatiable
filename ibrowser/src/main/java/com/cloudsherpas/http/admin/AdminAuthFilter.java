...
public class AdminAuthFilter extends CsFilter {
	...
	@Inject
	public AdminAuthFilter(Logger logger, UserService userService,
			UserDao userDao, GroupDao groupDao, GoogleGroupMemberDao memberDao) {
	...
	/*
	 * public boolean moreThanOneDay(Date date) { Calendar cal = new
	 * GregorianCalendar(); cal.setTime(new Date());
	 * cal.add(Calendar.DAY_OF_YEAR,-1); return cal.getTime().after(date); }
	 */
