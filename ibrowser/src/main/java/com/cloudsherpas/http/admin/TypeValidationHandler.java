...
public class TypeValidationHandler {
	...
	public void post(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		TypeValidationRequest tr = (TypeValidationRequest) utils.readPost(req, TypeValidationRequest.class);
		String key = tr.key;
		...
		if ("".equals(key) && groupCode != null && groupName!=null) {
			...
			if (editableType != null && editableType1!=null) {
				if (KeyFactory.keyToString(editableType.getKey()).equals(key) && KeyFactory.keyToString(editableType1.getKey()).equals(key)) {
					utils.sendResponse(resp, "bothNew");
				} else if(KeyFactory.keyToString(editableType.getKey()).equals(key) && !KeyFactory.keyToString(editableType1.getKey()).equals(key) ) {
					utils.sendResponse(resp, "groupNameExist");
				}else if(!KeyFactory.keyToString(editableType.getKey()).equals(key) && KeyFactory.keyToString(editableType1.getKey()).equals(key) ) {
					utils.sendResponse(resp, "groupCodeExist");
				}else{
