...
public abstract class BigQueryMapReduceServlet extends HttpServlet implements GlobalConstants {
  ...
  protected MapReduceSpecification<Entity, String, Entity, String, GoogleCloudStorageFileSet> createMapReduceSpec() {
    ...
    Output<String, GoogleCloudStorageFileSet> output = new MarshallingOutput<>(
        new GoogleCloudStorageFileOutput(GCS_BUCKET_NAME, getFilenamePattern(), "text/csv",
            getCloudStorageFileSetShard()),
        outputMarshaller);

    return MapReduceSpecification.of(getJobName(), input, mapper, intermediateKeyMarshaller,
        intermediateValueMarshaller, reducer, output);
  }

  protected void savePipelineHistory(PipelineService service, String pipelineId, String jobName)
...
//		return datastoreService.prepare(query).countEntities(FetchOptions.Builder.withDefaults());
  ...
  private class AppPipelineJob extends Job0<Void> {
    ...
    public Value<Void> run() throws Exception {
      ...
      FutureValue<MapReduceResult<GoogleCloudStorageFileSet>> mapReduceResult = futureCall(
          mapReduceJob, immediate(spec), immediate(settings));

      return futureCall(new GCSFilesToBigQueryJob(), mapReduceResult,
          immediate(beforeRemoveTable()));
    }

  }

  private class GCSFilesToBigQueryJob
      extends Job2<Void, MapReduceResult<GoogleCloudStorageFileSet>, Boolean> {
    private static final long serialVersionUID = 6277239748168293296L;

    @Override
    public Value<Void> run(MapReduceResult<GoogleCloudStorageFileSet> files,
        Boolean beforeRemoveTable) throws Exception {
      /*
       * List<String> sources = new ArrayList<>(); for (GcsFilename name :
       * files.getOutputResult().getAllFiles()) { sources.add("gs://" + name.getBucketName() + "/" +
       * name.getObjectName()); }
       * 
       * BigQueryApi api = new BigQueryApi(); Bigquery service = api.authenticateToBigQuery();
       * 
       * //remove a table with older data before creating the table with new data if
       * (beforeRemoveTable != null && beforeRemoveTable.booleanValue()) { try {
       * service.tables().delete(PROJECT_ID, DATASET_ID, getTableId()).execute(); } catch(Exception
       * e) { } }
       * 
       * Job job = new Job(); JobConfiguration config = new JobConfiguration(); JobConfigurationLoad
       * loadConfig = new JobConfigurationLoad(); config.setLoad(loadConfig);
       * 
       ...
       * 
       * // Describe the resulting table you are importing to: TableReference tableRef = new
       * TableReference(); tableRef.setDatasetId(DATASET_ID); tableRef.setTableId(getTableId());
       * tableRef.setProjectId(PROJECT_ID); loadConfig.setDestinationTable(tableRef);
       * 
       * TableSchema schema = new TableSchema(); schema.setFields(getFieldsSchema());
       * 
       * loadConfig.setSchema(schema); loadConfig.setFieldDelimiter(getFieldDelimiter());
       * 
       * 
       * Insert insert = service.jobs().insert(PROJECT_ID, job); insert.setProjectId(PROJECT_ID);
       * JobReference jobRef = insert.execute().getJobReference();
       * 
       * long startTime = System.currentTimeMillis(); long elapsedTime;
       * 
       * while (true) { Job pollJob = service.jobs().get(PROJECT_ID, jobRef.getJobId()).execute();
       * elapsedTime = System.currentTimeMillis() - startTime;
       * 
       * System.out.format("Job status (%dms) %s: %s\n", elapsedTime, jobRef.getJobId(),
       * pollJob.getStatus().getState());
       * 
       * if (pollJob.getStatus().getState().equals("DONE")) { break; } // Pause execution for one
       * second before polling job status again, to // reduce unnecessary calls to the BigQUery API
       * and lower overall // application bandwidth. Thread.sleep(1000); }
       */
      return null;
    }
